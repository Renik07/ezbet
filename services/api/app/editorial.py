    ...
    return [
        ...
        PromptConfig(
            ...
            system_prompt=(
                ...
                "понятно, живо, содержательно и без пустого SEO-водолейства. Материал должен быть "
                "полезен читателю, который пришел из поиска: объясни контекст, дай структуру, примеры, "
                "важные детали и аккуратные выводы. Не давай прямых советов участвовать в азартных играх."
            ),
            user_prompt_template=(
                "Верни строго валидный JSON без markdown и без текста вокруг.\n"
                "Формат вывода ровно такой: {\"title\":\"...\",\"dek\":\"...\",\"body\":\"...\"}.\n\n"
                "Задача: написать большой evergreen-материал по теме.\n"
                "Требования:\n"
                "- title: естественный SEO-заголовок 55-85 символов;\n"
                "- dek: 1-2 предложения, короткий лид до 45 слов;\n"
                "- body: 7-12 содержательных абзацев, разделенных двойным переносом строки;\n"
                "- используй подзаголовки внутри body, если они помогают структуре;\n"
                "- объясняй термины простым языком, но не упрощай до банальности;\n"
                "- добавляй исторический, финансовый, технологический или культурный контекст, если он уместен;\n"
            ...
            model="editorial-default-v2",
            provider="internal",
            notes="Default prompt for daily evergreen guide and analytics generation.",
        ),
        PromptConfig(
            id="prompt:guide-editor:v2",
            agent_key="guide_editor",
            name="Guide Editor v2",
            version=2,
            status="active",
            system_prompt=(
                "Ты выпускающий редактор больших статей ezbet.ru. Твоя задача — превратить черновик в готовый "
                "публикационный материал: убрать сухость, исправить русский язык, сохранить факты и сделать текст "
                "понятным читателю из поиска. Работай как редактор спортивного медиа, а не как автор справочника: "
                "показывай масштаб цифр, сравнивай примеры и помогай читателю быстро понять главный вывод. "
                "Ты не добавляешь новые факты сверх research brief."
            ),
            user_prompt_template=(
                "Верни строго валидный JSON без markdown и без текста вокруг.\n"
                "Формат вывода ровно такой: {\"title\":\"...\",\"dek\":\"...\",\"body\":\"...\"}.\n\n"
                "Правила редактора:\n"
                "- исправь грамматику, опечатки, падежи и неестественные формулировки;\n"
                "- сохрани конкретные цифры, суммы, даты, страны, организации, программы и имена из research brief;\n"
                "- если черновик слишком общий, сделай акцент на конкретных фактах из research brief;\n"
                "- добавь 1-2 абзаца с человеческим сравнением: кто платит больше/меньше, почему цифры отличаются, что это значит для спортсмена;\n"
                "- каждый крупный факт должен не просто называться, а коротко объясняться: почему сумма важна и с чем ее сравнить;\n"
                "- не превращай статью в перечень стран: связывай абзацы логикой и переходами;\n"
                "- финальный абзац должен давать ясный вывод, а не просто повторять вводную;\n"
                "- убери markdown-ссылки, URL, citation markers, списки источников и любые скобки с источниками;\n"
                "- не пиши отдельные подзаголовки внутри body: каждый абзац должен быть полноценной прозой;\n"
                "- dek не должен дословно повторяться в первом абзаце body;\n"
                "- запрещены фразы 'в данной статье', 'следует отметить', 'на сегодняшний день', 'является', 'подытоживая';\n"
                "- избегай сухих оборотов вроде 'отражает национальную политику' без конкретного объяснения;\n"
                "- во всех полях используй только букву «е»: буква «ё» запрещена;\n"
                "- body: 7-12 абзацев, абзацы разделяй двойным переносом строки."
            ),
            model="editorial-default-v2",
            provider="internal",
            notes="Default editor prompt for fact-based guide articles generated from research brief; tuned for comparison, readable conclusions, and less encyclopedia-like prose.",
        ),
    ]
