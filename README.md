...
При первом локальном запуске API создайте изолированное Python-окружение:

```sh
npm run setup:api
```

Дальше PostgreSQL и Redis запускаются через `npm run dev:infra`, а API с hot reload — через `npm run dev:api`.

Полный ручной прогон выполняется из корня проекта:

```sh
npm run forecasts:run
```

Команда загружает `.env`, выбирает до пяти ещё не начавшихся футбольных матчей текущей даты по Москве, генерирует прогнозы и сохраняет результат в PostgreSQL. Для production cron используйте шаблон `scripts/forecast-crontab.example`: он запускает процесс ежедневно в `01:00` с `CRON_TZ=Europe/Moscow`.
...
Требования:
- title: естественный новостной заголовок без кликбейта; обязательно переформулируй его и не повторяй source title дословно
- dek: 1-2 предложения, коротко и по делу
- body: пиши столько абзацев, сколько реально нужно по объему фактов; если новость короткая, достаточно 1-2 коротких абзацев; если есть содержательный full_text, обычно нужен более полный материал на 2-4 абзаца с сохранением основных фактов
- не добавляй факты, которых нет в source
- не используй фразы вроде 'данный материал', 'в рамках', 'следует отметить', 'на текущий момент', если они не нужны
- избегай AI-канцелярита и повторов
- если есть full_text, опирайся в первую очередь на него и не своди весь первоисточник к краткому пересказу из 1-2 фраз, если в нём есть несколько самостоятельных фактов
-не повторяй в body один и тот же факт или цитату два раза в разных формулировках
-Если один и тот же смысл уже был сказан, во втором абзаце добавь новый контекст (фон, реакцию, влияние на турнир), а не перефразируй старое
-Если исходный текст короткий и содержит 1–2 ключевых факта, достаточно 1–2 абзацев без повторов и «воды».

editor
Ты выпускающий редактор ezbet.ru. Проверяй новостной текст как живой редактор: смотри на точность, естественность, плотность фактов и отсутствие воды. Не переписывай ради красоты: твоя цель — понять, готов ли текст к публикации. 
